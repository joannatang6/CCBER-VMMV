...
recruits_col <- sept_recruits %>% 
  ...
  geom_col() +
  labs(title = "Recruitment", x = "Zone", y = "# seedlings", caption = "Graph shows number of seedlings in each zone in September 2020 \n No significant differences (chi-squared = 3.82, p = 0.15)") +
  theme_classic() +
  scale_y_continuous(expand = c(0,0)) +
  theme_classic()
recruits_col
...
recruits_per_plant_line <- summary_recruits_per_plant %>% 
  ...
  geom_point() +
  geom_line() +
  labs(title = "Recruitment per plant over time", x = "Date", y = "# seedlings per plant", caption = "Graph shows mean number of seedlings per plant over time") +
  theme_classic() +
  theme(axis.text.x = element_text(angle = 90)) +
  scale_x_date(date_labels = "%B 1", breaks = "1 month")
...
accessions_summary_recruits <- vmmv_recruits %>% 
  group_by(date, maternal_accession, lineage) %>% 
  filter(lineage != "NA") %>% 
  summarize(sum_seedlings = sum(total_seedlings))
...
accession_recruits_col <- accessions_summary_recruits %>% 
  filter(date == "2020-09-21") %>%
  ggplot(aes(x = maternal_accession, y = sum_seedlings, fill = as.factor(lineage))) +
  geom_col() +
  labs(title = "Recruitment", x = "Maternal Accession", y = "# seedlings", caption = "Graph shows number of seedlings from each accession in September 2020 \n Maternal accession has significant effect on recruitment (chi-squared = 18.13, p = 0.020)") +
  theme_classic() +
  scale_y_continuous(expand = c(0,0)) +
  scale_fill_grey(name = "Lineage") +
  theme_classic()
...
accession_recruits_per_plant_col <- vmmv_recruits %>% 
  ...
  group_by(date, maternal_accession, lineage) %>% 
  summarize(mean_seedlings_per_plant = mean(sum_seedlings)) %>% 
  ggplot(aes(x = maternal_accession, y = mean_seedlings_per_plant, fill = as.factor(lineage))) +
  geom_col() +
